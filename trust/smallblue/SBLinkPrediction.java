...
public class SBLinkPrediction {

	private static int MAX_TOTAL_SAMPLE_SIZE = 60000;

	private static int MAX_POSITIVE_SAMPLE_SIZE = 30000;

	private static SBLinkPrediction _instance = new SBLinkPrediction();

	private SBLinkPrediction() {
	}

	public static SBLinkPrediction getInstance() {
		return _instance;
	}
	...
	private static final LPNetwork TRAINING_NETWORK = LPNetwork.IBM_NEG;

	private static final LPNetwork TEST_NETWORK = LPNetwork.IBM_POS;

	private static final Month TRAINING_START_MONTH = Month.JAN;

	private static final Month TRAINING_END_MONTH = Month.AUG;

	private static final String TRAINING_PERIOD = Month.getMonthsRange(
			TRAINING_START_MONTH, TRAINING_END_MONTH);

	private static final String TEST_PERIOD = Month.getMonthsRange(Month.SEP,
			Month.DEC);
	...
	private void loadNodes() {
		...
		LoadSBNodeFeatures loadNodeFeatures = new LoadSBNodeFeatures();
		System.out
				.println("Step 1 "
						+ getIdString()
						+ " - Build static features of player nodes in training period.. ");
		loadNodeFeatures.loadNodes(TRAINING_START_MONTH, TRAINING_END_MONTH);
	}
	...
	private void loadEdgeSamples() {
		...
		LoadSBEdgeDataset loadEdgeDataset = new LoadSBEdgeDataset();
